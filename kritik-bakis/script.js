document.addEventListener("DOMContentLoaded", () => {
  ...
  nameInput.addEventListener("input", () => {
    ...
    nameElement.textContent = nameInput.value; // Her zaman büyük harfe çevir
  });

  // Unvan alanını güncelleme
  titleInput.addEventListener("input", () => {
    const titleElement = document.querySelector(".title");
    titleElement.textContent = titleInput.value; // Her zaman büyük harfe çevir
  });
  ...
  downloadButton.addEventListener("click", () => {
    ...
    backgroundImage.onload = () => {
      ...
      wrapText(ctx, textInput.value, 75, 240, 930, fontSize + 10); // Y koordinatını 260 olarak ayarladık

      // İsim
      const nameElement = document.querySelector(".name");
      const nameStyle = window.getComputedStyle(nameElement);
      ctx.font = `${nameStyle.fontSize} ${nameStyle.fontFamily}`;
      ctx.fillText(nameInput.value, 115, 965);

      // Unvan
      const titleElement = document.querySelector(".title");
      const titleStyle = window.getComputedStyle(titleElement);
      ctx.font = `${titleStyle.fontSize} ${titleStyle.fontFamily}`;
      ctx.fillText(titleInput.value, 115, 1000);
